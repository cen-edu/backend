...
class ProblemAuthoringModelComparisonLiveTest {
  ...
  @Test void comparesApprovedPairs() throws Exception {
    ...
    for(String g:List.of("gpt-4o-mini","gpt-5.6-luna")) if(generatorFilter.isBlank() || generatorFilter.equals(g))
      ...
      } catch (RuntimeException failure) { out.append(g).append('\t').append(v).append('\t').append(caseId).append("\tERROR\t").append(failure.getClass().getSimpleName()).append('\t').append(String.valueOf(failure.getMessage()).replaceAll("\\s+", " ")).append('\n'); }
    }
  ...
  private ModelClient open(String model){ String effort=model.startsWith("gpt-5")?"medium":"minimal"; OpenAiProperties p=new OpenAiProperties(System.getenv("OPENAI_API_KEY"),model,effort,1200,Duration.ofSeconds(60),0,Map.of()); OpenAiClientConfig c=new OpenAiClientConfig(); OpenAIClient raw=c.openAIClient(p); return new ModelClient(raw,new OpenAiLlmClient(c.openAiChatModel(raw,c.openAiChatOptions(p)),p)); }
