package com.cenedu.backend.domain.analysis.controller;

import static org.mockito.Mockito.when;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;

import java.util.List;

import com.cenedu.backend.domain.analysis.dto.response.AnalysisReportGenerationResponse;
import com.cenedu.backend.domain.analysis.dto.response.AnalysisReportResponse;
import com.cenedu.backend.domain.analysis.entity.enums.GenerationStatus;
import com.cenedu.backend.domain.analysis.service.AnalysisReportService;
import com.cenedu.backend.global.common.enums.UserRole;
import com.cenedu.backend.global.security.JwtProvider;
import com.cenedu.backend.support.PostgresTestcontainer;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.boot.webmvc.test.autoconfigure.AutoConfigureMockMvc;
import org.springframework.context.annotation.Import;
import org.springframework.test.context.bean.override.mockito.MockitoBean;
import org.springframework.test.web.servlet.MockMvc;

@SpringBootTest(properties = {
        "app.jwt.secret=cen-edu-analysis-report-controller-test-secret",
        "app.jwt.access-token-expiration=1h"
})
@AutoConfigureMockMvc
@Import(PostgresTestcontainer.class)
class AnalysisReportControllerTest {

    private static final String REPORT_PATH =
            "/api/teacher/analysis/assignments/101/students/11/report";

    @Autowired
    private MockMvc mockMvc;

    @Autowired
    private JwtProvider jwtProvider;

    @MockitoBean
    private AnalysisReportService reportService;

    @Test
    @DisplayName("생성 요청은 202와 생성 상태·재조회 간격을 반환한다")
    void requestsGeneration() throws Exception {
        when(reportService.requestGeneration(7L, 101L, 11L)).thenReturn(
                new AnalysisReportGenerationResponse(GenerationStatus.GENERATING, 3000L));

        mockMvc.perform(post(REPORT_PATH)
                        .header("Authorization", "Bearer " + teacherToken()))
                .andExpect(status().isAccepted())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.generationStatus").value("GENERATING"))
                .andExpect(jsonPath("$.data.retryAfterMs").value(3000));
    }

    @Test
    @DisplayName("조회는 생성 상태와 문장을 반환한다")
    void getsReport() throws Exception {
        when(reportService.getReport(7L, 101L, 11L)).thenReturn(
                new AnalysisReportResponse(
                        GenerationStatus.READY,
                        "요약",
                        null,
                        List.of(new AnalysisReportResponse.ItemMessage(
                                501L, "확인된 점", "학습 포인트", "다시 풀 때")),
                        "관찰"));

        mockMvc.perform(get(REPORT_PATH)
                        .header("Authorization", "Bearer " + teacherToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.generationStatus").value("READY"))
                .andExpect(jsonPath("$.data.summaryMessage").value("요약"))
                .andExpect(jsonPath("$.data.customLearningMessage").doesNotExist())
                .andExpect(jsonPath("$.data.itemMessages[0].worksheetItemId").value(501))
                .andExpect(jsonPath("$.data.overallObservation").value("관찰"));
    }

    @Test
    @DisplayName("JWT가 없으면 401을 반환한다")
    void rejectsUnauthenticatedRequest() throws Exception {
        mockMvc.perform(post(REPORT_PATH))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.error.code").value("UNAUTHORIZED"));
    }

    @Test
    @DisplayName("학생 JWT로 호출하면 403을 반환한다")
    void rejectsStudentJwt() throws Exception {
        String token = jwtProvider.issueAccessToken(11L, UserRole.STUDENT).value();

        mockMvc.perform(post(REPORT_PATH)
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.error.code").value("FORBIDDEN"));
    }

    private String teacherToken() {
        return jwtProvider.issueAccessToken(7L, UserRole.TEACHER).value();
    }
}
