...
--
-- 처음 V20260819_1000 으로 냈다가 problem 도메인 마이그레이션과 버전이 겹쳐 이 번호로 다시 낸다.
-- 옛 번호를 이미 적용한 DB 가 있어 모든 구문을 멱등으로 쓴다. 그런 DB 에서는 전부 no-op 이다.
ALTER TABLE worksheet
    ADD COLUMN IF NOT EXISTS parent_worksheet_id BIGINT;
...
-- parent_worksheet_id IS NULL 조건 덕분에 두 번 돌아도 결과가 같다.
UPDATE worksheet w
...
-- PostgreSQL 은 테이블 제약에 ADD CONSTRAINT IF NOT EXISTS 가 없다. 드롭 후 재생성으로 멱등을 만든다.
ALTER TABLE worksheet
    DROP CONSTRAINT IF EXISTS fk_worksheet_parent,
    DROP CONSTRAINT IF EXISTS ck_worksheet_parent_precedes;
...
CREATE INDEX IF NOT EXISTS idx_worksheet_parent ON worksheet (parent_worksheet_id);
