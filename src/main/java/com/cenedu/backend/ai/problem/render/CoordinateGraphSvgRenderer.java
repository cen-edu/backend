...
public final class CoordinateGraphSvgRenderer {
    ...
    public String render(CoordinateGraphDiagramSpecV1 s, Map<String, SemanticResolvedValue> v) {
        ...
        if (xt > 0) for (double x = Math.ceil(xmin / xt) * xt; x <= xmax; x += xt) {
            ...
            b.append("<line x1=\"").append(q).append("\" y1=\"").append(axisY - 3).append("\" x2=\"").append(q).append("\" y2=\"").append(axisY + 3).append("\" stroke=\"#000000\"/>");
        }
        if (yt > 0) for (double y = Math.ceil(ymin / yt) * yt; y <= ymax; y += yt) {
            int q = py(y, ymin, ymax, p, h - p);
            b.append("<line x1=\"").append(axisX - 3).append("\" y1=\"").append(q).append("\" x2=\"").append(axisX + 3).append("\" y2=\"").append(q).append("\" stroke=\"#000000\"/>");
        }
        for (var f : s.functions()) {
            double k = num(v, f.coefficientKey(), 1);
            if (f.functionKind() == CoordinateFunctionKind.INVERSE_PROPORTION) {
                b.append(path(f, p, w, h, xmin, xmax, ymin, ymax, k, Double.NEGATIVE_INFINITY, 0));
                b.append(path(f, p, w, h, xmin, xmax, ymin, ymax, k, 0, Double.POSITIVE_INFINITY));
            } else b.append(path(f, p, w, h, xmin, xmax, ymin, ymax, k, xmin, xmax));
        }
        for (var point : s.points()) {
            double x = num(v, point.xKey(), 0), y = num(v, point.yKey(), 0);
            b.append("<circle cx=\"").append(px(x, xmin, xmax, p, w - p)).append("\" cy=\"").append(py(y, ymin, ymax, p, h - p)).append("\" r=\"4\" fill=\"#000000\"/>");
        }
        return b.toString();
    }

    private String path(CoordinateFunctionSpec f, int p, int w, int h, double xmin, double xmax, double ymin, double ymax, double k, double from, double to) {
        StringBuilder d = new StringBuilder();
        int n = 64;
        for (int i = 0; i <= n; i++) {
            double x = from == Double.NEGATIVE_INFINITY ? xmin + (0 - xmin) * i / (double) n : to == Double.POSITIVE_INFINITY ? 0 + (xmax) * i / (double) n : from + (to - from) * i / (double) n;
        ...
        return d.isEmpty() ? "" : "<path d=\"" + d + "\" fill=\"none\" stroke=\"#000000\"/>";
    }

    private int px(double x, double min, double max, int a, int b) {
        return (int) Math.round(a + (x - min) / (max - min) * (b - a));
    }

    private int py(double y, double min, double max, int a, int b) {
        return (int) Math.round(b - (y - min) / (max - min) * (b - a));
    }
