...
public final class PlaneGeometrySvgRenderer {
    ...
    public String render(PlaneGeometryDiagramSpecV1 s, Map<String, SemanticResolvedValue> v) {
        ...
        for(var q:s.polygons()){StringBuilder d=new StringBuilder();for(String k:q.pointKeys()){int[] z=points.get(k);if(z!=null)d.append(d.isEmpty()?"M":" L").append(z[0]).append(' ').append(z[1]);}if(!d.isEmpty()){b.append("<path d=\"").append(d).append(" Z\" fill=\"").append(q.filled()?"#EEEEEE":"none").append("\" stroke=\"#000000\"/>");int[] first=points.get(q.pointKeys().get(0));if(first!=null)label(b,first[0]+8,first[1]+16,q.labelTemplate());}}
