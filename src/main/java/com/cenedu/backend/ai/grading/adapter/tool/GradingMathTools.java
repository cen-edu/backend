...
public class GradingMathTools {
    ...
    private MathResult evaluateGuarded(String expression, Map<String, Double> variables) {
        ...
        MathResult rejected = scanShape(expression);
        if (rejected != null) {
            return rejected;
        }

        Map<String, Double> values = variables == null ? Map.of() : variables;
        Set<String> undefined = undefinedVariables(expression, values);
        ...
        try {
            if (expressionEvaluator.hasComparison(expression)) {
                return MathResult.ok(expressionEvaluator.evaluateComparison(expression, values));
            }
            double value = expressionEvaluator.evaluate(expression, values);
            if (!Double.isFinite(value)) {
                // 무한대·NaN 을 값으로 돌려주면 JSON 으로 실을 수 없다. 가드에 걸린 것으로 본다.
                return MathResult.failure(Status.TOO_COMPLEX, "계산 결과가 유한하지 않다.");
            }
    ...
    /**
     * 한 번 훑어 문자·깊이·부호 연속을 본다. 파서와 같은 순서로 읽어야 깊이가 맞는다 —
    ...
    private MathResult scanShape(String expression) {
        ...
        for (int i = 0; i < expression.length(); i++) {
            ...
            switch (c) {
                ...
                case ' ' -> {
                    // 파서는 공백을 건너뛰지 않는다. 모양 검사에서도 상태를 바꾸지 않고 넘긴다.
                }
                default -> {
                    if (!isAllowedAtom(c)) {
