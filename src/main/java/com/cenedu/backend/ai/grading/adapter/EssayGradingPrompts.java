 ...
 *
 * <p><b>가중치도 주지 않는다(D23).</b> 이유가 둘이다. 하나는 점수를 백엔드가 계산하므로(D16)
 * 모델이 알 이유가 없다는 것이고, 다른 하나가 더 중요하다 — <b>배점을 알면 판정이 배점 쪽으로
 * 기운다.</b> 40점짜리 항목과 10점짜리 항목을 같은 눈으로 보지 않게 되고, 그렇게 기운 판정은
 * 점수를 백엔드가 계산해도 이미 분자가 오염된 뒤다.
 *
 * <p>대가는 있다. 모델은 어느 항목이 중요한지 모르므로 판정 노력을 항목마다 균등하게 쓴다.
 * 그것이 이 설계가 의도한 바다 — 항목별 판정은 균등하게, 경중은 백엔드의 가중치가 매긴다.
 *
