...
public class EssayScoreCalculator {
    ...
    public enum Reason {
         ...
         *
         * <p><b>실패가 아니라 정상 종점이다(D25).</b> 일반 학습지의 서술형은 점수 없이 판정
         * 결과만 쓴다 — 호출부는 이 값을 받으면 점수를 비운 채 채점 완료로 기록한다. 실패로
         * 두면 그 학습지는 전 칸 {@code GRADED} 를 영영 만족하지 못해 확정이 열리지 않는다.
         */
        NO_MAX_SCORE
    }
     ...
     *                   값이 나온다. 일반 학습지의 서술형은 점수를 쓰지 않기로 확정됐다(D25)
