...
public class ProblemAssetStorageTask extends BaseTimeEntity {
    ...
    private ProblemAssetStorageTask(ProblemAsset asset, String sourceLocalPath, String targetStorageKey,
        ...
        this.status = ProblemAssetStorageStatus.PENDING;
        this.attemptCount = 0;
    }
    ...
    public void start(LocalDateTime leaseUntil) {
        status = ProblemAssetStorageStatus.PROCESSING;
        attemptCount++;
    ...
    public void complete() {
        status = ProblemAssetStorageStatus.READY;
        nextAttemptAt = null;
        lastErrorCode = null;
        asset.markReady();
    }
     ...
     * 재시도 가능한 실패를 기록한다.
     */
    public void retry(String code, LocalDateTime next) {
        status = ProblemAssetStorageStatus.RETRY_WAIT;
        lastErrorCode = code;
        nextAttemptAt = next;
        asset.markFailed(true);
    }
    ...
    public void fail(String code) {
        status = ProblemAssetStorageStatus.FAILED;
        lastErrorCode = code;
        asset.markFailed(false);
    }

    /**
     * 보존기간이 지난 영구 실패 작업의 로컬 원본이 정리됐음을 기록한다.
     */
    public void markSourceDeleted(LocalDateTime deletedAt) {
