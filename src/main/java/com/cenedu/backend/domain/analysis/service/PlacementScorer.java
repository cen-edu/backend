 ...
 * 배점(하 1점, 중 2점, 상 3점)으로 환산해 가중 성취도를 낸다. 문항 수 비율이 아니라 배점 비율로
 * 나누기 때문에 고배점 문항을 맞힌 학생이 제대로 위로 간다.
 *
 * <p>컷오프는 70% / 40% 다. 40% 미만을 하로 보내는 것은 문항반응이론의 기본 추측도(20~25%)를
 * 고려하면 그 구간의 정답이 실력을 뜻한다고 보기 어렵기 때문이다.
 *
 * <h2>진단지가 한 가지 난이도만으로 출제된 경우</h2>
 *
 * <p>가중 공식은 혼합 출제를 전제한다. 전부 '중'으로 출제된 진단지에서는 가중치가 상수가 되어
 * 성취도가 그냥 정답률이 되는데, 여기에 70% 컷오프를 그대로 적용하면 중 난이도만 풀고 75% 맞힌
 * 학생이 '상'으로 올라간다. 푼 적 없는 난이도를 실력으로 인정하는 셈이다.
 *
 * <p>그래서 단일 난이도일 때는 절대 컷오프 대신 <b>출제된 난이도를 기준으로 한 상대 조정</b>을
 * 한다. 정답률에 4.1 의 상태 판정 컷오프(80% / 40%)를 적용해 그 난이도에서 한 칸 올리거나
 * 내린다. 관찰된 것이 "중 난이도를 잘 푼다"이므로 결론도 "중의 한 칸 위"까지만 간다.
 */
public final class PlacementScorer {

    /** 혼합 진단지의 상 난이도 배정 컷오프(%). */
    public static final BigDecimal HIGH_CUTOFF = BigDecimal.valueOf(70);

    /** 혼합 진단지의 중 난이도 배정 컷오프(%). 미만이면 하로 배정한다. */
    public static final BigDecimal MID_CUTOFF = BigDecimal.valueOf(40);
    ...
    public static PlacementResult score(PlacementTally tally) {
        int maxScore = DifficultyLadder.LOW * tally.lowTotal()
                + DifficultyLadder.MID * tally.midTotal()
                + DifficultyLadder.HIGH * tally.highTotal();
        if (maxScore == 0) {
            return null;
        }

        int earnedScore = DifficultyLadder.LOW * tally.lowCorrect()
                + DifficultyLadder.MID * tally.midCorrect()
                + DifficultyLadder.HIGH * tally.highCorrect();
    ...
    public record PlacementResult(
        ...
        /** 단일 난이도 진단에서 실제로 적용된 상태 판정. 혼합이면 의미가 없다. */
        public MasteryStatus statusForSingleBand() {
            return MasteryStatusJudge.judgeByRate(rate);
        }
    }
}
