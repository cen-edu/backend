...
public final class ReissueSelector {
     ...
     *
     * <p><b>재출제는 박스형(STEP_FILL) 문항만 낸다.</b> 그런데 여기에 문항 유형 조건이 없다.
     * 지금은 {@link QuestionBank}가 읽는 파일이 단계형 뱅크 하나뿐이라 4,028문항이 전부
     * 박스형이어서, 조건을 걸지 않아도 결과가 같기 때문이다. 뱅크 원본에는 유형 필드조차
     * 없다.
     *
     * <p><b>문제은행 테이블로 옮길 때 반드시 하드 필터에 유형을 추가해야 한다.</b> 그쪽에는
     * 객관식·주관식·서술형이 함께 있어서, 조건 없이 고르면 재출제에 서술형이 섞여 나간다.
     * 그때 이 자리가 비어 있으면 빠뜨리기 쉬우므로 미리 적어 둔다.
     *
