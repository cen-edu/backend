...
**에이전트가 `AgentDispatcher`를 주입받지 마세요.** 디스패처는 생성자에서 모든 `Agent` 빈을 모으기 때문에, 에이전트가 디스패처를 요구하면 서로 기다리다 기동이 실패합니다 (`BeanCurrentlyInCreationException`).

| 하고 싶은 것 | 어디에 두나 |
|---|---|
| 여러 에이전트가 같은 로직을 쓴다 | `Agent`가 **아닌** 별도 `@Component`로 빼고 각자 주입받습니다 |
| 에이전트 호출 여러 개를 순서대로 엮는다 | 도메인 서비스가 `dispatch()`를 여러 번 부릅니다 |
| 모든 에이전트에 공통으로 적용할 검사다 | `InputGuard` / `OutputGuard` |

조율은 도메인의 일이지 에이전트의 일이 아닙니다. 서비스가 엮으면 각 호출이 대등하게 가드레일을 통과하고, `userInput`이 사용자가 실제로 입력한 문장이라는 전제도 유지됩니다. 에이전트 안에서 다시 부르면 그 전제가 깨져서 — 두 번째 호출의 `userInput`은 LLM이 만들어낸 문자열입니다 — 가드레일 판정의 의미가 불분명해집니다.
