...
- **HTTPS 는 붙이지 않는다.** 이 배포는 학습·시연용이다. 그래서 **실제 학생의 이름·답안을
  넣지 않는 것이 전제 조건이다.** 지금 구성은 JWT 와 답안 이미지 URL 이 평문으로 오간다.
  실데이터를 다루게 되는 순간 이 결정을 먼저 뒤집어야 한다(6절).
...
| 컨테이너 | 첫 측정(512m/256m) | 두 번째(384m/192m) | 최종 상한 |
|---|---|---|---|
| backend | 408MB (80%) | 369MB (**96%**) | **448m** (힙 336MB) |
| postgres | 119MB | 40MB | **192m** + shared_buffers 64MB |
| frontend (nginx) | 2.5MB | 4.9MB | **32m** |
| 시스템 스왑 사용 | 410MB | 138MB | — |

384m 은 상한에 붙어(96%) OOM 여지가 생겨 되돌렸다. 컨테이너가 OOM 으로 죽으면
`restart: always` 가 되살리지만 그동안 화면이 멈춘다 — 스왑을 조금 쓰는 편이 낫다.
