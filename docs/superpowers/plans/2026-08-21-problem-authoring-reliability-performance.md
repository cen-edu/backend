...
- 각 단계 완료 시 관련 테스트를 실행한다. 전체 `bash gradlew build`는 Task7 측정·문서화가 끝난 뒤 사용자에게 실행 시점을 안내하고, 사용자가 직접 실행한다.
...
- [ ] **Step 4: 생성 모델과 검증 모델 분리 실험을 수행한다**
  ...
  비용을 고려해 생성·검증 모델은 `gpt-4o-mini`와 `gpt-5.6-luna`로만 제한한다. 생성×검증 2×2 조합을 같은 표본에서 비교하고, 오탐·미탐·비용·지연을 함께 평가한다. 다른 모델은 비교하거나 설정 후보로 추가하지 않는다.
...
- [ ] **Step 6: 최종 빌드 실행 시점을 안내한다**

  Task7 구현·측정·결과 문서화까지 완료한 뒤 `bash gradlew build`는 실행하지 않고 사용자에게 직접 실행할 시점을 안내한다.

  사용자가 직접 실행할 최종 검증: 모든 단위·통합·ArchUnit 테스트 PASS.
...
- Task7 측정·결과 문서화가 완료되고, 최종 `bash gradlew build` 실행 시점이 사용자에게 안내됐다.
